...
/*
 * tiny_push_back is the implementation detail that rearanges the
 * elements in tiny based on pattern patching.
 */
template <class Tiny, class T, int Size = mpl::size<Tiny>::value>
...
struct mpl::push_back_impl<tiny_tag>
{
    ...
    struct apply
    {
        typedef typename tiny_push_back<
            Tiny, 
            T
        >::type type;
    };
