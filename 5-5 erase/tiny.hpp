...
struct mpl::empty_impl<tiny_tag>
{
    template <class Tiny>
    struct apply
        : mpl::equal_to<typename mpl::size<Tiny>::type, mpl::integral_c<int, 0>>
    {};
};
...
struct mpl::begin_impl<tiny_tag>
{
    template <class Tiny>
    struct apply
        : tiny_iterator<Tiny, mpl::integral_c<int, 0>>
    {};
};
...
static_assert(
    std::is_same<
        ...
        tiny_iterator<tiny123, mpl::integral_c<int, 3>>
    >::value
);
...
struct mpl::next<tiny_iterator<Tiny, Pos>>
{
    typedef tiny_iterator<
        Tiny,
        // plus will convert Pos to integral_c
        // unlike mpl::next
        // This way advance and next produce the same iterators
        typename mpl::plus<Pos, mpl::int_<1>>::type
    > type;
};
...
struct mpl::prior<tiny_iterator<Tiny, Pos>>
{
    typedef tiny_iterator<
        Tiny,
        // Look for mpl::next implementation for the why mpl::minus is here
        typename mpl::minus<Pos, mpl::int_<1>>::type
    > type;
};
...
/*
 * tiny_insert is a helper template that will shift the values based on the
 * Index where the type shall be inserted
 */
...
/*
 * mpl::insert will insert a type, so that the provided pointer will point
 * to it after the operation.
 */
template <>
